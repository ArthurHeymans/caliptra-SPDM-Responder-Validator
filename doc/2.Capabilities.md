...
Assertion 2.7.10:
    Not recorded. SPDM 1.3 removes the MUT_AUTH_CAP dependency on ENCAP_CAP and defines no replacement dependency.
...
Steps:
1. Requester -> GET_CAPABILITIES {SPDMVersion=0x14, Param1.SupportedAlgorithms=1, Param2=0, CTExponent, ExtFlags=0, Flags=CERT_CAP|CHAL_CAP|ENCRYPT_CAP|MAC_CAP|MUT_AUTH_CAP|KEY_EX_CAP|ENCAP_CAP|HBEAT_CAP|KEY_UPD_CAP|CHUNK_CAP|LARGE_RESP_CAP, DataTransferSize, MaxSPDMmsgSize}
2. SpdmMessage <- Responder

Assertions 2.8.1 through 2.8.15 correspond to the SPDM 1.3 capability assertions from Case 2.7, with the following version-specific values.
...
Assertion 2.8.10:
    Not recorded. SPDM 1.4 defines no MUT_AUTH_CAP dependency.
...
Assertion 2.8.16:
    (Flags & ~SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_14_MASK) == 0

Assertion 2.8.17:
    (ExtFlags & ~SPDM_GET_CAPABILITIES_EXTENDED_RESPONSE_FLAGS_14_MASK) == 0

Assertion 2.8.18:
    if (Flags.SET_KEY_PAIR_RESET_CAP == 1), then (Flags.SET_KEY_PAIR_INFO_CAP == 1)

Assertion 2.8.19:
    If Param1.SupportedAlgorithms == 0, no Supported Algorithms block is present. If it is 1, the block length and reserved fields are valid, Param1 is at most 6, and each AlgStructure has a valid, unique, strictly increasing AlgType. AlgCount.FixedAlgCount is 2 for AlgType 0x02 through 0x05. For AlgType 0x06 through 0x07, FixedAlgCount is nonzero and ExtAlgCount is 0. The block length includes every extended algorithm entry.

The test does not require SLOT_MGMT_CAP, SET_KEY_PAIR_RESET_CAP, LARGE_RESP_CAP, or a Supported Algorithms block to be advertised. It validates these fields only when present.
