...
Description: SPDM responder shall return valid MEASUREMENTS, if it receives a GET_MEASUREMENTS with negotiated version 1.4.

SPDM Version: 1.4

This case mirrors case 7.9 with the connection negotiated at SPDM version 1.4. Every GET_MEASUREMENTS request carries the 8-byte RequesterContext introduced in SPDM 1.3, and each MEASUREMENTS response must echo it between OpaqueData and Signature.

Assertions are identical to 7.9.1 ~ 7.9.21, plus:

Assertion 7.11.22 / 7.11.23 / 7.11.24:
    SpdmMessage.RequesterContext == GET_MEASUREMENTS.RequesterContext (for the number, all, and one-by-one exchanges respectively)

SPDMsignatureVerify uses the version 1.4 signing context.

### Case 7.12

Description: SPDM responder shall return valid MEASUREMENTS in a DHE session, if it receives a GET_MEASUREMENTS with negotiated version 1.4 in a DHE session.

SPDM Version: 1.4

This case mirrors case 7.10 at SPDM version 1.4, with the same RequesterContext handling as case 7.11.
